...
def LeptonsSelection_dev(df):
    df = df.Define(
        "Muon_acceptanceSel", f"""(v_ops::pt(Muon_p4) > 15 && abs(v_ops::eta(Muon_p4)) < 2.4)"""
    )
    ...
    df = df.Filter(
        "Muon_idx[Muon_acceptanceSel && Big_ID_Iso_OR].size()==2",
        "Consider events with exactly 2 muons",
    )
    df = df.Define("mu1_idx", "Muon_idx[Muon_acceptanceSel && Big_ID_Iso_OR][0]")
    df = df.Define("mu2_idx", "Muon_idx[Muon_acceptanceSel && Big_ID_Iso_OR][1]")
